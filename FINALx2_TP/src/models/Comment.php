...
class Comment {
  ...
  public function findAll ($postId) {
    $query = "SELECT c.id, c.contenu, u.nom as nom_utilisateur, c.date_commentaire FROM comments c JOIN users u ON u.id = c.utilisateur_id WHERE c.post_id = :post_id ORDER BY c.date_commentaire DESC";
    $stmt = $this->conn->prepare($query);
    $stmt->bindParam(':post_id', $postId);
    $stmt->execute();
